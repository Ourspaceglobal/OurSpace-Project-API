...
class SupportTicketSubmittedNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage())
            ->subject("You opened support ticket: {$this->supportTicket->reference}")
            ->line('We acknowledge receipt of your support ticket.')
    ...
    public function toArray($notifiable)
    {
        return [
            'status' => true,
            'message' => "You opened support ticket: {$this->supportTicket->reference}",
        ];
    }
}
